...
make_install_talloc:
	...
	$(CD) ./talloc-2.1.9 && \
		$(ENV) PATH=/usr/$(BUILD_HOST)/bin:/usr/bin:/usr/sbin:/bin:/sbin CC=$(CC) CPP=$(CPP) AR=$(AR) RANLIB=$(RANLIB) STRIP=$(STRIP) LD=$(LD) OBJCOPY=$(OBJCOPY) OBJDUMP=$(OBJDUMP) \
...
make_install_proot:
	...
	$(CD) ./proot-termux-git && \
		...
		($(CD) ./src && \
			$(ENV) PATH=/usr/$(BUILD_HOST)/bin:/usr/bin:/usr/sbin:/bin:/sbin LC_ALL=C $(MAKE) -f ./GNUmakefile \
				CROSS_COMPILE=$(CROSS_COMPILE) CC=$(CC) LD=$(CC) STRIP=$(STRIP) OBJCOPY=$(OBJCOPY) \
				OBJDUMP=$(OBJDUMP) CPPFLAGS="-D_FILE_OFFSET_BITS=64 -D_GNU_SOURCE -I. $(CPPFLAGS)" \
				CFLAGS="-Wall -Wextra -O2 $(CFLAGS)" LDFLAGS="$(LDFLAGS) -static -ltalloc -Wl,-z,noexecstack" V=1 && \
				$(STRIP) ./proot)
